...
export async function POST(request: Request) {
  try {
    ...
    if (!Array.isArray(prompts) || prompts.length === 0) {
      return Response.json({ error: "Invalid prompts array" }, { status: 400 });
    }
    ...
    const combinedPrompt = content
      .replace(
        /^Here['']s a combined super-prompt that incorporates the key elements from all the provided prompts:\n*/i,
        ""
      )
